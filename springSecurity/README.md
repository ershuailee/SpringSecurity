...
执行 [spring_security.sql](sql/spring_security.sql) 脚本完成数据库创建、表结构和基础数据的初始化。

### 2. 配置修改

[application.yml](src/main/resources/application.yaml) 修改MySQL、Redis连接配置；

### 3. 后端启动
执行 [SystemApplication.java](src/main/java/com/system/SystemApplication.java) 的 main 方法完成后端项目启动；
...
参考 ([Angular](https://github.com/conventional-changelog/conventional-changelog/tree/master/packages/conventional-changelog-angular)) 社区规范
